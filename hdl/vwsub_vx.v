// Compute widening signed vector-scalar subtraction between a vector of length XLEN bits with a SEW bits size (single element width) and a scalar of SEW-width length, resulting in a 2*SEW bit output
    ...
    case(sew)
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin
                        if (Ovflw16[j]) S16[j] = (temp16[j] < 0) ? 32'h80000000 : 32'h7FFFFFFF; // Saturate if overflow
                        else S16[j] = rounded16[j];
                    end else if (!vma) S16[j] = S16_old[j]; // Retain old value
                    else S16[j] = 32'hFFFFFFFF; // Mark agnostic
                end else if (vta) S16[j] = 32'hFFFFFFFF; // Tail agnostic fill
            end

            for (j = 0; j < vecwidth/2; j = j + 1)
                S[32*j +: 32] = S16[j]; // Pack result vector
        end
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                Ovflw32[j] = (Aext32 > 0 && B32[j] > 0 && temp32[j] < 0) || (Aext32 < 0 && B32[j] < 0 && temp32[j] >= 0); // Overflow detection

                if (j < vl) begin
                    if (vxsat) begin // Fixed-point rounding
                        case (vxrm)
                    ...
                    end else if (!vma) S32[j] = S32_old[j]; // Retain old
                    else S32[j] = 64'hFFFFFFFFFFFFFFFF; // Agnostic
                end else if (vta) S32[j] = 64'hFFFFFFFFFFFFFFFF; // Tail fill
            end

            for (j = 0; j < vecwidth/2; j = j + 1)
                S[64*j +: 64] = S32[j]; // Pack widened 64-bit results
        end
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                Ovflw64[j] = (Aext64 > 0 && B64[j] > 0 && temp64[j] < 0) || (Aext64 < 0 && B64[j] < 0 && temp64[j] >= 0); // Overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin
                        if (Ovflw64[j]) S64[j] = (temp64[j] < 0) ? 128'h80000000000000000000000000000000 : 128'h7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // Saturate if overflow
                        else S64[j] = rounded64[j];
                    end else if (!vma) S64[j] = S64_old[j]; // Retain old
                    else S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // Mark as agnostic
                end else if (vta) S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // Tail fill
            end

            for (j = 0; j < vecwidth/2; j = j + 1)
                S[128*j +: 128] = S64[j]; // Pack widened 128-bit results
        end
    endcase
end
