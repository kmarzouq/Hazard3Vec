// Compute widening signed vector-vector addition between two vectors of length XLEN bits and various SEW bits (single element width), resulting in a 2*SEW bit output
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S16[j] = S16_old[j]; // if set is marked agnostic
                    else S16[j] = 32'hFFFFFFFF;
                end else if (vta) S16[j] = 32'hFFFFFFFF; // tail agnostic
            end

            for (j = 0; j < vecwidth/2; j = j + 1) // output writeback
                S[32*j +: 32] = S16[j];
        end
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S32[j] = S32_old[j]; // agnostic behavior
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S64[j] = S64_old[j]; // agnostic
