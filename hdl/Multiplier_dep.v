`include "multiplier_32bit_stable.v"
...
wire [MAX_VECWIDTH*64-1:0] mul_raw;
wire [MAX_VECWIDTH*64-1:0] overflow_flags;

// Parallel instantiation of the 32-bit adders
genvar i;
generate
    for (i = 0; i < MAX_VECWIDTH; i=i+1) begin : VMUL_LOOP
        wire [XLEN-1:0] a_lane;
        wire [XLEN-1:0] b_lane;
        wire [2*XLEN-1:0] p_lane;
        wire ovflw_lane;

        assign a_lane = A[XLEN*i +: XLEN];
        assign b_lane = B[XLEN*i +: XLEN];
