// Compute widening unsigned vector-scalar addition between a vector of length XLEN bits with a SEW bits size (single element width) and a scalar of SEW-width length, resulting in a 2*SEW bit output
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S16[j] = S16_old[j]; // If set is marked agnostic, retain previous value
                    else S16[j] = 32'hFFFFFFFF; // Agnostic set to all 1's
                end else if (vta) S16[j] = 32'hFFFFFFFF; // Tail agnostic elements overwritten as 1's
            end

            for (j = 0; j < vecwidth/2; j = j + 1) // Output result
                S[32*j +: 32] = S16[j];
        end
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S32[j] = S32_old[j]; // If set is marked agnostic, retain previous value
                    else S32[j] = 64'hFFFFFFFFFFFFFFFF; // Agnostic set to all 1's
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                if (j < vl) begin
                    ...
                    end else if (!vma) S64[j] = S64_old[j]; // If set is marked agnostic, retain previous value
                    else S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // Agnostic set to all 1's
