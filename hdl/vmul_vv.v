// Compute signed vector-vector multiplication between vectors of length XLEN bits with a SEW bits size (single element width). Returns the lower bits of the product
    ...
    case(sew)
        8: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                P8m[j] = A8m[j] * B8m[j]; // multiplication for each 8-bit element
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                P64m[j] = A64m[j] * B64m[j]; // magnitude multiplication
                temp64[j] = Sign64Out[j] ? ~P64m[j] + 1 : P64m[j]; // apply sign to result
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        S64[j] = rounded64[j]; // no overflow clamp logic here
