// Compute vector-vector subtraction between two vectors of length XLEN bits and various SEW bits (single element width)
    ...
    case(sew)
        8: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp8[j] = A8[j] - B8[j]; //subtraction for each 8-bit element
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp16[j] = A16[j] - B16[j]; //subtraction for each 16-bit element
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp32[j] = A32[j] - B32[j]; //subtraction for each 32-bit element
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp64[j] = A64[j] - B64[j]; //subtraction for each 64-bit element
