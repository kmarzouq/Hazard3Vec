// Compute widening signed vector-vector subtraction between two vectors of length XLEN bits and various SEW bits (single element width), resulting in a 2*SEW bit output
...
// funct6: 110011 (vwsub) for Vector Instruction Listing
...
//when sew = 8
reg signed [7:0] A8 [MAX_VECWIDTH-1:0];
...
//when sew = 16
reg signed [15:0] A16    [MAX_VECWIDTH-1:0];
...
//when sew = 32
reg signed [31:0] A32    [MAX_VECWIDTH-1:0];
...
//when sew = 64
reg signed [63:0] A64    [MAX_VECWIDTH-1:0];
    ...
    case(sew)
        8: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp8[j] = A8[j] - B8[j]; // widening subtraction (16-bit result)
                Ovflw8[j] = (A8[j] > 0 && B8[j] > 0 && temp8[j] < 0) || (A8[j] < 0 && B8[j] < 0 && temp8[j] >= 0); // signed overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw8[j]) S8[j] = (temp8[j] < 0) ? 16'h8000 : 16'h7FFF; // Replace if overflow occurs
                        else S8[j] = rounded8[j];
                    end else if (!vma) S8[j] = S8_old[j]; // if set is marked agnostic, retain previous value or overwrite
                    else S8[j] = 16'hFFFF;
                end else if (vta) S8[j] = 16'hFFFF; // tail agnostic elements overwritten as 1's
            end

            for (j = 0; j < vecwidth/2; j = j + 1) // output consisting 1/2*vecwidth*SEW sized register to account for widened result
                S[16*j +: 16] = S8[j];
        end
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp16[j] = A16[j] - B16[j]; // widening subtraction (32-bit result)
                Ovflw16[j] = (A16[j] > 0 && B16[j] > 0 && temp16[j] < 0) || (A16[j] < 0 && B16[j] < 0 && temp16[j] >= 0); // signed overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw16[j]) S16[j] = (temp16[j] < 0) ? 32'h80000000 : 32'h7FFFFFFF; // Replace if overflow occurs
                        else S16[j] = rounded16[j];
                    end else if (!vma) S16[j] = S16_old[j]; // if set is marked agnostic, retain previous value or overwrite
                    else S16[j] = 32'hFFFFFFFF;
                end else if (vta) S16[j] = 32'hFFFFFFFF; // tail agnostic elements overwritten as 1's
            end
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp32[j] = A32[j] - B32[j]; // widening subtraction (64-bit result)
                Ovflw32[j] = (A32[j] > 0 && B32[j] > 0 && temp32[j] < 0) || (A32[j] < 0 && B32[j] < 0 && temp32[j] >= 0); // signed overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw32[j]) S32[j] = (temp32[j] < 0) ? 64'h8000000000000000 : 64'h7FFFFFFFFFFFFFFF; // Replace if overflow occurs
                        else S32[j] = rounded32[j];
                    end else if (!vma) S32[j] = S32_old[j]; // if set is marked agnostic, retain previous value or overwrite
                    else S32[j] = 64'hFFFFFFFFFFFFFFFF;
                end else if (vta) S32[j] = 64'hFFFFFFFFFFFFFFFF; // tail agnostic elements overwritten as 1's
            end
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp64[j] = A64[j] - B64[j]; // widening subtraction (128-bit result)
                Ovflw64[j] = (A64[j] > 0 && B64[j] > 0 && temp64[j] < 0) || (A64[j] < 0 && B64[j] < 0 && temp64[j] >= 0); // signed overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw64[j]) S64[j] = (temp64[j] < 0) ? 128'h80000000000000000000000000000000 : 128'h7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // Replace if overflow occurs
                        else S64[j] = rounded64[j];
                    end else if (!vma) S64[j] = S64_old[j]; // if set is marked agnostic, retain previous value or overwrite
                    else S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
                end else if (vta) S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // tail agnostic elements overwritten as 1's
            end
