// Compute widening unsigned vector-vector addition between two vectors of length XLEN bits and various SEW bits (single element width), resulting in a 2*SEW bit output
...
// funct6: 110000 (vwaddu) for Vector Instruction Listing
    ...
    case(sew)
        8: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                Ovflw8[j] = (temp8[j] < A8[j]) || (temp8[j] < B8[j]); // unsigned overflow detection
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp16[j] = A16[j] + B16[j]; // widening addition (32-bit result)
                Ovflw16[j] = (temp16[j] < A16[j]) || (temp16[j] < B16[j]); // unsigned overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw16[j] && vxsat) S16[j] = 32'hFFFFFFFF; // saturation on overflow
            ...
            for (j = 0; j < vecwidth/2; j = j + 1) // output consisting vecwidth*SEW sized register
                S[32*j +: 32] = S16[j];
        end
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp32[j] = A32[j] + B32[j]; // widening addition (64-bit result)
                Ovflw32[j] = (temp32[j] < A32[j]) || (temp32[j] < B32[j]); // unsigned overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw32[j] && vxsat) S32[j] = 64'hFFFFFFFFFFFFFFFF; // saturation on overflow
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp64[j] = A64[j] + B64[j]; // widening addition (128-bit result)
                Ovflw64[j] = (temp64[j] < A64[j]) || (temp64[j] < B64[j]); // unsigned overflow detection
                ...
                if (j < vl) begin
                    ...
                    if (mask_out[j]) begin // determines which SEW elements to mask out
                        if (Ovflw64[j] && vxsat) S64[j] = 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF; // saturation on overflow
