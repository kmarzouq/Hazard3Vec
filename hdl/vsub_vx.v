// Compute vector-scalar subtraction between a vector of length XLEN bits with a SEW bits size (single element width) and a scalar of SEW-width length
...
// funct6: 000010 (vsub) for Vector Instruction Listing
...
reg signed [7:0] S8 [MAX_VECWIDTH-1:0];
reg signed [7:0] S8_old [MAX_VECWIDTH-1:0];
reg signed [7:0] temp8 [MAX_VECWIDTH-1:0];
reg signed [7:0] rounded8 [MAX_VECWIDTH-1:0];

//when sew = 16
reg signed [15:0] A16    [MAX_VECWIDTH-1:0];
reg signed [15:0] B16    [MAX_VECWIDTH-1:0];
reg signed [15:0] S16    [MAX_VECWIDTH-1:0];
reg signed [15:0] S16_old[MAX_VECWIDTH-1:0];
reg signed [15:0] temp16 [MAX_VECWIDTH-1:0];
reg signed [15:0] rounded16 [MAX_VECWIDTH-1:0];

//when sew = 32
reg signed [31:0] A32    [MAX_VECWIDTH-1:0];
reg signed [31:0] B32    [MAX_VECWIDTH-1:0];
reg signed [31:0] S32    [MAX_VECWIDTH-1:0];
reg signed [31:0] S32_old[MAX_VECWIDTH-1:0];
reg signed [31:0] temp32 [MAX_VECWIDTH-1:0];
reg signed [31:0] rounded32 [MAX_VECWIDTH-1:0];

//when sew = 64
reg signed [63:0] A64    [MAX_VECWIDTH-1:0];
reg signed [63:0] B64    [MAX_VECWIDTH-1:0];
reg signed [63:0] S64    [MAX_VECWIDTH-1:0];
reg signed [63:0] S64_old[MAX_VECWIDTH-1:0];
reg signed [63:0] temp64 [MAX_VECWIDTH-1:0];
reg signed [63:0] rounded64 [MAX_VECWIDTH-1:0];
    ...
    case(sew)
        8: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp8[j] = B8[j] - Aext8; //subtraction for each 8-bit element and scalar
        ...
        16: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp16[j] = B16[j] - Aext16; //subtraction for each 16-bit element and scalar
        ...
        32: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp32[j] = B32[j] - Aext32; //subtraction for each 32-bit element and scalar
        ...
        64: begin
            for (j = 0; j < vecwidth; j = j + 1) begin
                ...
                temp64[j] = B64[j] - Aext64; //subtraction for each 64-bit element and scalar
