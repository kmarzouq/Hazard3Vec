module vadd_vv #( parameter MAX_VECWIDTH=8 //Maximum LMUL-supported vector width
)(
  input clk,
  input reset,
...
//adder32bitby32 #(.MAX_VECWIDTH(MAX_VECWIDTH)) dut (.Cout(Cout), .S(temp), .A(A), .B(B), .Cin(Cin), .Ovflw(Ovflw), .vecwidth(vecwidth));
